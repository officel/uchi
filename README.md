...
`uchi` では、用途に応じて出力レベルを調整できます。

- **通常 (Normal)**: サブコマンドの標準的な結果および設定概要などを出力します。
- **詳細 (`-v`, `--verbose`)**: 解析対象のドキュメント、抽出・スキップされたコードブロックとその理由（`uchi: v1` 未指定、未アノテーション、対象シェル不一致など）、生成計画の詳細ログを出力します。
- **静寂 (`-q`, `--quiet`)**: 成功時の非必須な情報表示（`check` の設定一覧や `new`/`init` の作成メッセージ）を抑制します（`gen --dry-run` や `diff` などの機械可読なコマンド結果は維持されます）。
- **同時指定**: `--verbose` と `--quiet` の同時指定はエラーとなります。
