...
- **`output_dir`**: `../dist`
- **`shell`**: `all`（対象シェル生成フィルタ、既定値: `all`）
- **`auto_comment`**: `true`（自動ヘッダーコメント付与機能が有効）

### 対象シェルの選別 (Target Shell Filtering)

- **優先順位**: CLI フラグ (`-s` / `--shell`) > 設定ファイル (`shell: <target>`) > 既定値 (`all`)
- `uchi gen` 実行時、選択された `shell` に一致するコードフェンスのみを抽出して生成計画を構築します。
  - フェンスの `target` 属性未指定（既定値 `all`）または `target=all` のブロックは、すべての選択シェルに適合します。
  - 特定のシェルが宣言されたブロック（例: `target=bash` や `target="bash,zsh"`）は、選択された対象シェルが含まれる場合のみ出力されます。選ばれなかったブロックが別の対象シェルに流用されることはありません。
- **未知のシェル**: `all`, `bash`, `fish`, `powershell`, `pwsh`, `sh`, `zsh` 以外の未知のシェル指定は生成処理の開始前に診断エラーで拒否されます。
- **空の生成結果**: 選択したシェルに該当するコードブロックが存在せず出力が空となる場合、エラーとはならず 0 件の出力ファイル生成として正常終了（終了コード 0）します。
